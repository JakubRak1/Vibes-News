...
The successful launch of the Perseverance Rover is a testament to the ingenuity and determination of the human spirit, and is sure to inspire future generations of scientists and explorers.''', source='Washington Post', category_id = db.session.query(Category).filter_by(name='News').first().id, user_id = db.session.query(User).filter_by(username = 'emilytaylor').first().id )
...
This is a momentous occasion in the fight against cancer and marks a significant step forward in the search for a cure.''', source='The Guardian', category_id = db.session.query(Category).filter_by(name='News').first().id, user_id = db.session.query(User).filter_by(username = 'matthewr').first().id)
...
The successful launch of the Tianwen-1 Rover marks a major milestone in China's space program and is a testament to the country's technological advancements and commitment to scientific exploration.''', source='Xinhua News', category_id = db.session.query(Category).filter_by(name='News').first().id, user_id =  db.session.query(User).filter_by(username = 'oliviah').first().id)
...
This is a major step forward in the field of quantum computing and marks a significant moment in the history of computing.''', source='Science Daily', category_id = db.session.query(Category).filter_by(name='News').first().id, user_id =  db.session.query(User).filter_by(username = 'oliviah').first().id)
...
The Lakers' championship win caps off a season unlike any other, with the league playing through the pandemic and facing a host of new challenges. But through it all, the Lakers proved to be the best team in the league, and their championship is a testament to their talent, perseverance, and determination.''', source='ESPN', category_id = db.session.query(Category).filter_by(name='Sport').first().id, user_id = db.session.query(User).filter_by(username = 'emilytaylor').first().id)
...
The win is a testament to Williams' talent, determination, and resilience, and it solidifies her place as one of the greatest tennis players of all time. It is a moment that will be remembered for years to come and a true tribute to Williams' incredible career.''', source='The New York Times', category_id = db.session.query(Category).filter_by(name='Sport').first().id, user_id = db.session.query(User).filter_by(username = 'jamesg').first().id)
...
The world record-breaking run was a moment that will be remembered for years to come, and it stands as a testament to Bolt's incredible talent and determination.''', source='NBA News', category_id = db.session.query(Category).filter_by(name='Sport').first().id, user_id = db.session.query(User).filter_by(username = 'jamesg').first().id)
...
The victory was a moment that will be remembered for years to come, and it stands as a testament to Woods' talent, resilience, and determination.''', source='CNN Sports', category_id = db.session.query(Category).filter_by(name='Sport').first().id, user_id = db.session.query(User).filter_by(username = 'jamesg').first().id)
...
The acquisition is part of Amazon's larger strategy to dominate the streaming market, and it will give them a valuable new source of content as they compete with other major players in the industry. The future of Amazon's media and entertainment offerings looks bright, and this acquisition is a strong step forward for the company.''', source='The Wall Street Journal', category_id = db.session.query(Category).filter_by(name='Buissnes').first().id, user_id =  db.session.query(User).filter_by(username = 'jamesg').first().id)
...
The new factory will be a major investment for Tesla, and it will allow the company to continue their growth and expansion in the years to come. The future of electric vehicles looks bright, and Tesla is well-positioned to lead the way as the industry continues to grow and evolve.''', source='The New York Times', category_id = db.session.query(Category).filter_by(name='Buissnes').first().id, user_id =  db.session.query(User).filter_by(username = 'matthewr').first().id)
...
The new iPhone will be available for pre-order later this week, and it is expected to hit stores next month. With its improved features and sleek design, it is sure to be a must-have device for tech enthusiasts and Apple fans alike.''', source='CNBC', category_id= db.session.query(Category).filter_by(name='Buissnes').first().id, user_id = db.session.query(User).filter_by(username = 'matthewr').first().id)
...
The return of vinyl is a testament to the timeless appeal of this classic format, and it is a reminder that in an age of digital music, there is still a place for the warmth and richness of analog sound.''', source='Rolling Stone', category_id = db.session.query(Category).filter_by(name='Culture').first().id, user_id = db.session.query(User).filter_by(username = 'jamesg').first().id)
...
"Poi is a truly unique and beautiful art form," says Laura Jenkins, a poi performer and instructor. "It's about more than just swinging balls – it's about the flow, the movement, and the connection that you feel with the audience. Whether you're a seasoned poi performer or just getting started, there is something truly special about this art form.''', source='The Performing Arts Journal', category_id = db.session.query(Category).filter_by(name='Culture').first().id, user_id = db.session.query(User).filter_by(username = 'matthewr').first().id)
...
"Esports is the future of gaming," says James Taylor, an esports commentator and analyst. "From the high-stakes tournaments to the live streams and the community of players, it's a truly thrilling and engaging experience that is taking the world by storm.''', source='Gaming News Daily', category_id = db.session.query(Category).filter_by(name='Game News').first().id, user_id = db.session.query(User).filter_by(username = 'oliviah').first().id)
...
"Mobile gaming is the future of gaming," says John Smith, a mobile game designer and entrepreneur. "From the convenience and accessibility to the competitive and social aspects, it's a truly exciting time to be a mobile gamer.''', source='Mobile Gaming Today', category_id = db.session.query(Category).filter_by(name='Game News').first().id, user_id = db.session.query(User).filter_by(username = 'oliviah').first().id)
...
"The future of console gaming is bright," says Tom Green, a console game developer and analyst. "From the latest hardware to the cutting-edge games, the next generation of console gaming is set to take the gaming world by storm.''', source='Console Gaming Weekly', category_id =  db.session.query(Category).filter_by(name='Game News').first().id, user_id = db.session.query(User).filter_by(username = 'matthewr').first().id)
