...
def manage_users_editor():
    ...
    if (current_user.is_authenticated and current_user.admin_rights == 1):
        ...
        for category in current_user.category:
            users_cat: list[User] = db.session.query(User).filter(User.category.any(Category.name == category.name), User.admin_rights == 0).all()
            users += users_cat
...
def manage_article_editor():
    ...
    if (current_user.is_authenticated and current_user.admin_rights == 1):
        ...
        for category in current_user.category:
            article_cat: list[Article] = db.session.query(Article).filter(Article.category_id == category.id).all()
